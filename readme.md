...
모든 무점검 경고(uncheked warning)는 가능하다면 없애야 코드의 형 안전성이 보장된다. 제거할 수 없는 경고 메시지는 형 안정성이 확실할 때에만 `@SuperWarnings("unchekced")` 어노테이션을 사용해 억제한다. 이 어노테이션을 사용할 때 마다, 왜 형 안정성을 위반하지 않는지 밝히는 주석을 반드시 붙이라. 형 안전성에 대한 증명도 없이 경고를 억제하면 프로그램 안정성에 대해 그릇된 생각만 갖게 된다.